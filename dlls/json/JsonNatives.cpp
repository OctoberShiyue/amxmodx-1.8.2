...
static cell AMX_NATIVE_CALL amxx_json_get_string(AMX* amx, cell* params)
{
	...
	return  MF_SetAmxString(amx, params[2], string == NULL ? "" : string, strlen(string));//MF_SetAmxStringUTF8Char(amx, params[2], string, strlen(string), params[3])
}
...
static cell AMX_NATIVE_CALL amxx_json_array_get_string(AMX* amx, cell* params)
{
	...
	return MF_SetAmxString(amx, params[3], string == NULL ? "" : string, strlen(string));//MF_SetAmxStringUTF8Char(amx, params[3], string, strlen(string), params[4])
}
...
static cell AMX_NATIVE_CALL amxx_json_object_get_name(AMX* amx, cell* params)
{
	...
	return MF_SetAmxString(amx, params[3], name == NULL ? "" : name, strlen(name)); //MF_SetAmxStringUTF8Char(amx, params[3], name, strlen(name), params[4])
}
...
static cell AMX_NATIVE_CALL amxx_json_serial_to_string(AMX* amx, cell* params)
{
	...
	auto written = MF_SetAmxString(amx, params[2], result == NULL ? "" : result, strlen(result));//(result) ? MF_SetAmxStringUTF8Char(amx, params[2], result, strlen(result), params[3]) : 0
...
void OnAmxxAttach()
{
	...
	MF_PrintSrvConsole("JSON_LOAD_RUN_BY_SHIYUE:QQ1023261581\n");
